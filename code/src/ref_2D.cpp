static char help[] ="Solves the 2D reflection equation u_t + a*A*u_x + b*A*u_y = 0, A = [0,1;1,0].";
...
{ 
  ...
  auto b = [](const PetscInt i, const PetscInt j){ return 2;};
  ...
  DMDACreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,
               Nx,Ny,PETSC_DECIDE,PETSC_DECIDE,2,stencil_radius,NULL,NULL,&da);
  DMSetFromOptions(da);
  DMSetUp(da);
  ...
  set_initial_condition(da, v_analytic, &appctx);
  VecSet(v_error,0);
...
PetscErrorCode set_initial_condition(DM da, Vec v, AppCtx *appctx) 
{
  ...
  for (j = appctx->i_start[1]; j < appctx->i_end[1]; j++)
  {
    ...
    for (i = appctx->i_start[0]; i < appctx->i_end[0]; i++)
    {
      ...
      varr[j][i][0] = theta2(x,y,0) - theta1(x,y,0);
      varr[j][i][1] = theta2(x,y,0) + theta1(x,y,0);
    }
  }
