...
namespace sbp{
  ...
  **/
  ...
  /**
  * Approximate RHS of 2D a general advection problem, u_t = -(au_x+bu_y), between indices in i_start, i_end.
  * Inputs: D1        - SBP D1 operator
  ...
  **/
  template <class SbpDerivative, typename VelocityFunction>
  inline PetscErrorCode advection_apply_2D(const SbpDerivative& D1, 
                                           VelocityFunction&& a,
                                           VelocityFunction&& b,
                                           const PetscScalar *const *const *const array_src,
                                           PetscScalar *const *const *const array_dst,
