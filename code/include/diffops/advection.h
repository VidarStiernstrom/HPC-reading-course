...
namespace sbp{
  ...
  **/
  ...
  inline PetscErrorCode advection_apply(const SbpDerivative& D1, PetscScalar **array_src, PetscScalar **array_dst, PetscInt i_start, PetscInt i_end, PetscInt N, PetscScalar hi);
  ...
  inline PetscErrorCode advection_apply(const SbpDerivative& D1, PetscScalar **array_src, PetscScalar **array_dst, PetscInt i_start, PetscInt i_end, PetscInt N, PetscScalar hi)
  {
    ...
    if (i_start == 0) {
      ...
      { 
        array_dst[i][0] = -D1.apply_left(array_src,hi,i,0);
      }
      i_start = n_closures;
    }
    ...
    if (i_end == N) {
      ...
      {
          array_dst[i][0] = -D1.apply_right(array_src,hi,N,i,0);
      }
      i_end = N-n_closures;
    }
    ...
    {
      array_dst[i][0] = -D1.apply_interior(array_src,hi,i,0);
    }
